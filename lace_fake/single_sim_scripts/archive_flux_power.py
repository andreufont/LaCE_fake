...
parser.add_argument(
    "--post_dir",
    type=str,
    help="Base directory with simulation post-processings",
    required=True,
)
parser.add_argument("--snap_num", type=int, help="Snapshop number", required=True)
parser.add_argument(
    "--axis", type=int, help="Axis to extract skewers (1,2,3)", required=False
)
parser.add_argument(
    "--n_skewers",
    type=int,
    default=10,
    help="Number of skewers per side",
    required=False,
)
parser.add_argument(
    "--width_Mpc", type=float, default=0.1, help="Cell width (in Mpc)", required=False
)
parser.add_argument(
    "--scales_tau",
    type=str,
    default="1.0",
    help="Comma-separated list of optical depth scalings to use.",
    required=False,
)
parser.add_argument(
    "--p1d_label",
    type=str,
    default=None,
    help="String identifying P1D measurement and / or tau scaling.",
    required=False,
)
parser.add_argument(
    "--add_p3d",
    action="store_true",
    help="Compute also P3D from skewers grid",
    required=False,
)
parser.add_argument(
    "--verbose", action="store_true", help="Print runtime information", required=False
)
...
snap_filename = skewers_dir + extract_skewers.get_snapshot_json_filename(
    num=args.snap_num, n_skewers=args.n_skewers, width_Mpc=args.width_Mpc
)
if verbose:
...
snapshot = snapshot_admin.SnapshotAdmin(
    snap_filename, scales_tau=scales_tau, kF_Mpc=kF_Mpc, post_dir=post_dir, axis=axis
)
